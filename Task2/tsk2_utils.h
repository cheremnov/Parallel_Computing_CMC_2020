 ...
 */
void printHelp(){
    std::cout << "tsk2 FILE_NAME [-d]" << std::endl;
...
int readMatrixParametersFile( const char* file_name, 
    ...
    size_t row_len, column_len, divided, not_divided, block_rows, block_columns;
    ...
    if( block_rows > row_len || block_columns > column_len ){
        std::cout << "Decomposition exceeds boundaries" << std::endl;
        return -1;
    }
